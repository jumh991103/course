...
- 내부 DB(상품·영양소) 조회는 이미 있는 서비스(product_service/nutrient_service)를
  Tool로 감싸서 그대로 재사용한다(RAG가 아니라 단순 키워드 조회).
- 외부 조회는 TavilySearch로 "뉴스"와 "회수·안전성 경보" 두 개 Tool만 추가한다.
- 두 Tool 모두 조회 권한만 가지며, 실행(구매취소·신고접수 등)은 하지 않는다.
"""
import re
...
from app.services import nutrient_service, product_service

_URL_PATTERN = re.compile(r"https?://[^\s\"'<>]+")
# search_recall_alert 도구가 실제로 회수·경보를 찾았을 때만 심는 내부 표식(사용자에게는 노출되지 않는다).
_RECALL_FOUND_MARKER = "[[RECALL_FOUND]]"

# day04 설계문서 System Prompt 확장 섹션을 그대로 반영한다.
_SYSTEM_PROMPT = """당신은 건강기능식품 추천 서비스 '건강한하루'의 AI 상담원입니다.

원칙:
1. 내부 데이터(search_internal_product, search_internal_nutrient)로 답할 수 있으면 내부 데이터를 우선 사용한다.
2. 최신 뉴스, 리콜/회수 여부, 트렌드처럼 내부 데이터에 없는 질문만
   search_supplement_news 또는 search_recall_alert 도구를 사용한다.
3. 도구를 사용했다면 반드시 검색 결과의 출처 URL을 함께 제시한다.
4. 도구 결과에도 없는 내용은 확신하듯 말하지 않는다. 모르면 모른다고 답한다.
...
def _build_internal_tools():
    ...
    @tool
    def search_internal_product(키워드: str) -> str:
        """상품명·주요원료·관련증상 키워드로 내부 상품 DB를 검색합니다. 항상 가장 먼저 시도해야 합니다."""
        db = SessionLocal()
        try:
            products = product_service.list_products(db, 키워드=키워드)
        finally:
            db.close()
        if not products:
            return f"내부 DB에서 '{키워드}'와(과) 관련된 상품을 찾지 못했습니다."
        lines = [f"'{키워드}' 관련 내부 DB 상품 (최대 5건):"]
        for p in products[:5]:
            lines.append(f"- {p.상품명} ({p.카테고리}, {p.업소명}): {p.관련_증상}")
        return "\n".join(lines)

    @tool
    def search_internal_nutrient(영양소: str) -> str:
        """영양소 이름으로 내부 DB의 기능성·근거 정보를 조회합니다. 성분 설명 질문에는 항상 먼저 시도해야 합니다."""
        db = SessionLocal()
        try:
            explanation = nutrient_service.get_nutrient_explanation(db, 영양소)
        finally:
            db.close()
        if explanation is None:
            return f"내부 DB에서 '{영양소}'에 대한 정보를 찾지 못했습니다."
        lines = [f"'{영양소}' 내부 DB 기능성 정보:"]
        for f in explanation.기능성_목록:
            lines.append(f"- {f.기능성} (근거: {f.근거_설명 or '정보 없음'}, 출처: {f.출처 or '정보 없음'})")
        return "\n".join(lines)

    return [search_internal_product, search_internal_nutrient]
...
def ask(message: str, history: list[ChatMessage]) -> ChatResponse:
    ...
    return ChatResponse(answer=answer, 출처_URL_목록=sources[:5], cs_상담_권장=cs_recommended)
