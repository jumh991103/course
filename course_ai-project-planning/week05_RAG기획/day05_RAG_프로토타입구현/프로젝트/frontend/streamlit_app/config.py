...
# NFR-1.0 참고: 추천 2초 이내 응답이 기준이므로, 그보다 넉넉하게 타임아웃을 둔다.
API_TIMEOUT_SECONDS = 8

# RF-3 AI상담(Agent)은 내부 RAG 검색(로컬 Ollama 임베딩) + LLM(Groq) + 외부 Tool(Tavily)
# 호출을 순차적으로 여러 번 수행할 수 있어 다른 API보다 오래 걸린다. 특히 로컬 Ollama
# 임베딩 모델은 컴퓨터 사양(CPU/GPU, 메모리)에 따라 첫 호출(모델을 메모리에 올리는 시점)
# 시간 차이가 크므로, 실습 환경마다 여유 있게 타임아웃을 잡아야 한다.
# 내 컴퓨터에서 계속 "요청이 지연되고 있어요" 오류가 뜬다면, 아래처럼 환경변수로
# 타임아웃을 늘려서 실행해 보자.
#   Windows PowerShell: $env:HEALTHY_DAY_CHAT_TIMEOUT_SECONDS = "60"
CHAT_TIMEOUT_SECONDS = int(os.environ.get("HEALTHY_DAY_CHAT_TIMEOUT_SECONDS", "60"))
