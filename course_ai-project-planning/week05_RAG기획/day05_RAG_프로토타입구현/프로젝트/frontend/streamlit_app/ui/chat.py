"""AI 상담 (UI-W-005) — RF-3, RAG(벡터 검색)를 사용하는 AI Agent.

week04 day04 "AI 에이전트 챗봇 설계(외부 인터넷 조회 도구)" 문서의 구조에
week05에서 만든 RAG(벡터 검색)를 결합한 최종 버전이다. 내부 지식베이스
(상품·영양소 기능성·섭취기준)를 벡터 검색으로 먼저 찾아보고, 최신 뉴스·리콜·트렌드
질문은 backend의 Agent가 TavilySearch Tool을 호출해 답한다. 회수·경보가 발견되면
...
def render() -> None:
    st.subheader("AI 상담")
    st.caption(
        "내부 지식베이스(상품·영양소)를 벡터 검색(RAG)으로 먼저 찾아보고, 최신 뉴스·리콜/회수 여부처럼 "
        "내부에 없는 정보는 인터넷을 조회해서 답해드려요."
    )
    ...
    for i, turn in enumerate(st.session_state.chat_messages):
        with st.chat_message(turn["role"]):
            ...
            if turn.get("references"):
                with st.expander("📎 근거 문서 (내부 지식베이스)"):
                    for ref in turn["references"]:
                        st.markdown(f"- {ref}")
            if turn.get("sources"):
                with st.expander("🌐 출처 (외부 검색)"):
