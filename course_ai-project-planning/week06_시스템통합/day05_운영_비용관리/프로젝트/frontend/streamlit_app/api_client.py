...
from config import API_BASE_URL, API_TIMEOUT_SECONDS, CHAT_TIMEOUT_SECONDS
...
def _request(method: str, path: str, timeout: int = API_TIMEOUT_SECONDS, **kwargs):
...
# ── RF-4: 운영 대시보드 (토큰 사용량/비용) ──────────
def get_usage_summary(days: int = 14) -> dict:
    return _request("GET", "/usage/summary", params={"days": days})


def get_usage_alerts() -> dict:
    return _request("GET", "/usage/alerts")
