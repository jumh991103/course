...
# ── RF-4 운영 대시보드(토큰 사용량/비용) 테스트 공통 fixture ────────────
@pytest.fixture
def usage_log_cleanup():
    """테스트가 llm_usage_log에 남긴 행만 정확히 지운다.

    실제 운영 중에 쌓인 기록(dev DB를 공유하는 경우)을 건드리지 않도록,
    테스트 시작 시점의 최댓값(id)을 기억해뒀다가 그보다 큰 id만 지운다.
    """
    ...
    db = SessionLocal()
    try:
        max_id_before = db.query(func.max(UsageLog.id)).scalar() or 0
    finally:
        db.close()
    ...
    db = SessionLocal()
    try:
        db.query(UsageLog).filter(UsageLog.id > max_id_before).delete()
        db.commit()
    finally:
        db.close()
...
_TS_NAMES = {
    ...
    "11": "LLM 호출마다 토큰 사용량/비용이 자동으로 기록된다",
    "12": "일별 토큰/비용 사용량이 정확히 집계된다",
    "13": "비용·토큰·오류율 알림 기준을 넘으면 알림이 발생한다",
}
